...
module.exports = async (req, res) => {
  // Auth : header Authorization OU query ?secret= (Vercel cron utilise le header)
  ...
  try {
    ...
    for (const c of cards) {
      // setCode et number obligatoires pour la clé unique
    ...
    // 3. Upsert par batches (la clé de conflit est l'unique (set_code, card_number, language))
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      const batch = rows.slice(i, i + BATCH_SIZE);
      const { error } = await supabase
        .from('lorcana_cardmarket_mapping')
        .upsert(batch, { onConflict: 'set_code,card_number,language' });
